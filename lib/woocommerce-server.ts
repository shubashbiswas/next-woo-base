// WooCommerce REST API Functions (SERVER-ONLY)
// This file contains all API fetch functions that use Node.js-specific modules (undici, Buffer).
// It must NOT be imported from client components ("use client").
// Client components should import from `lib/woocommerce.ts` instead.
...
// HTTP connection pool for reusing TCP connections to WooCommerce
const wcAgent = new Agent({
  connections: 10,
...
// Uses query parameter authentication (consumer_key + consumer_secret)
// instead of Basic Auth header because some WC installations reject
// Authorization headers on self-signed or local HTTPS setups.
function buildWooCommerceUrl(
  ...
  // Append credentials as query params
...
// In development mode, logs the actual error to help diagnose connectivity issues
async function woocommerceFetchGraceful<T>(
  ...
  if (!isConfigured) {
    if (process.env.NODE_ENV === "development") {
      console.error(`WooCommerce not configured - missing env vars. Check WORDPRESS_URL, WC_CONSUMER_KEY, WC_CONSUMER_SECRET`);
    }
    return fallback;
  }
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error(`[WooCommerce] Fetch failed for ${endpoint}:`, error);
      if (error instanceof WooCommerceAPIError) {
        console.error(`[WooCommerce]   Status: ${error.status}`);
        console.error(`[WooCommerce]   Endpoint: ${buildWooCommerceUrl(endpoint, query)}`);
...
async function woocommerceFetchPaginatedGraceful<T>(
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development") {
      console.error(`[WooCommerce] Paginated fetch failed for ${endpoint}:`, error);
      const url = buildWooCommerceUrl(endpoint, query);
      console.error(`[WooCommerce]   URL: ${url}`);
      console.error(`[WooCommerce]   Check that:`);
      console.error(`[WooCommerce]     1. WordPress is running at ${baseUrl}`);
      console.error(`[WooCommerce]     2. WooCommerce is activated`);
      console.error(`[WooCommerce]     3. REST API is enabled (Settings > Permalinks > Post name)`);
      console.error(`[WooCommerce]     4. Consumer key/secret have read permissions`);
      console.error(`[WooCommerce]     5. No IP restrictions on the REST API`);
    }
    return emptyResponse;
  }
}
